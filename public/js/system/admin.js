 ...
 * @date 2016-01-11 17:01:36
 * @last-modified-by tanshaohui
 * @last-modified-time 2016-01-12 12:54:32
 */
...
$('form').on('submit', function (e) {
    var uname = $('#uname').val();
    var password = md5(uname + $('#password').val());
    $.post('/api/admin/login', {
        uname: uname,
        password: password
    },function (data) {
        if (data.errno === 2) {
            $('#uname').focus().parents('.form-group').addClass('has-error');
            $('[role="alert"]').text(data.msg).addClass('alert-warning');
        } else if (data.errno === 3) {
            $('#password').focus().parents('.form-group').addClass('has-error');
            $('[role="alert"]').text(data.msg + '，今天还有' + data.ret + '次机会').addClass('alert-warning');
...
$('#uname').on('change', function () {
    ...
    $('[role="alert"]').text('').removeClass('alert-warning');
});
